...
MyViewer::MyViewer(QWidget* parent) :
	...
	show_control_points(true), show_solid(true), show_wireframe(false), show_constraints(false), show_partitioning(false),
	boundaryRemeshL(1.0),
...
void MyViewer::updateVertexNormalsConstraint() {
	...
	//   N. Max, Weights for computing vertex normals from facet normals.
	//     Journal of Graphics Tools, Vol. 4(2), 1999.
	for (auto v : keepInContsraint.vertices()) {
		Vector n(0.0, 0.0, 0.0);
		for (auto h : keepInContsraint.vih_range(v)) {
			if (keepInContsraint.is_boundary(h))
				continue;
			auto in_vec = keepInContsraint.calc_edge_vector(h);
			auto out_vec = keepInContsraint.calc_edge_vector(keepInContsraint.next_halfedge_handle(h));
			double w = in_vec.sqrnorm() * out_vec.sqrnorm();
			n += (in_vec % out_vec) / (w == 0.0 ? 1.0 : w);
		}
		double len = n.length();
		if (len != 0.0)
			n /= len;
		keepInContsraint.set_normal(v, n);
	}
	for (auto v : keepOutConstraint.vertices()) {
		Vector n(0.0, 0.0, 0.0);
		for (auto h : keepOutConstraint.vih_range(v)) {
			if (keepOutConstraint.is_boundary(h))
				continue;
			auto in_vec = keepOutConstraint.calc_edge_vector(h);
			auto out_vec = keepOutConstraint.calc_edge_vector(keepOutConstraint.next_halfedge_handle(h));
			double w = in_vec.sqrnorm() * out_vec.sqrnorm();
			n += (in_vec % out_vec) / (w == 0.0 ? 1.0 : w);
		}
		double len = n.length();
...
void MyViewer::setupCamera() {
	...
	if (!keepInContsraint.vertices_empty())
	{
		for (auto v : keepInContsraint.vertices()) {
			box_min.minimize(keepInContsraint.point(v));
			box_max.maximize(keepInContsraint.point(v));
		}
	}
	if (!keepOutConstraint.vertices_empty())
	{
		for (auto v : keepOutConstraint.vertices()) {
			box_min.minimize(keepOutConstraint.point(v));
			box_max.maximize(keepOutConstraint.point(v));
		}
	}
...
bool MyViewer::openGenerative(const std::string& filename, bool update_view) {
	...
	if (f)
	{
		...
		if (std::getline(f, line)) {
			openMesh(folder + "/" + line);
			if (std::getline(f, line) && line.compare(std::string(".")) == 0)
				newMesh = true;
			qDebug() << line.data();
		}
		else
			return false;
		while (newMesh && std::getline(f, line))
		{
			newMesh = false;
			std::string meshfile = folder + "/" + line;

			std::getline(f, line);

			//Type
			if (line.compare(std::string("circle")) == 0) {
				//qDebug() << "circle";
				CircleSolid* tempCircle = new CircleSolid();
				...
				int tempIdx = line.find_first_of(",");
				double x = std::stod(line.substr(0, tempIdx));
				int tempIdx2 = line.find_last_of(",");
				double y = std::stod(line.substr(tempIdx + 1, tempIdx2 - tempIdx));
				double z = std::stod(line.substr(tempIdx2 + 1, std::string::npos));
				tempCircle->setCenter(Vector(x, y, z));

				//radius

				std::getline(f, line);
				tempIdx = line.find_first_of("\t");
				double r = std::stod(line.substr(tempIdx, std::string::npos));
				tempCircle->setR(r);
				...
				line = line.substr(line.find_first_of("[") + 1, std::string::npos);
				tempIdx = line.find_first_of(",");
				double axisX = std::stod(line.substr(0, tempIdx));
				tempIdx2 = line.find_last_of(",");
				double axisY = std::stod(line.substr(tempIdx + 1, tempIdx2 - tempIdx));
				double axisZ = std::stod(line.substr(tempIdx2 + 1, std::string::npos));
				tempCircle->setAxis(Vector(axisX, axisY, axisZ));

				//Keep-in

				std::getline(f, line);
				tempIdx = line.find_first_of("\t");
				int keepIn = std::stoi(line.substr(tempIdx, std::string::npos));
				tempCircle->setKeepIn(keepIn == 1);

				//Keep-out

				std::getline(f, line);
				tempIdx = line.find_first_of("\t");
				int keepOut = std::stoi(line.substr(tempIdx, std::string::npos));
				tempCircle->setKeepOut(keepOut == 1);
				...
				constraint_solids.push_back(tempCircle);
				if (std::getline(f, line) && line.compare(std::string(".")) == 0)
					newMesh = true;
	...
	keepInContsraint.request_face_normals(); keepInContsraint.request_vertex_normals();
	keepInContsraint.update_face_normals();
	keepOutConstraint.request_face_normals(); keepOutConstraint.request_vertex_normals();
	keepOutConstraint.update_face_normals();

	updateVertexNormalsConstraint();
	setupCamera();
	//int n = buffer.str().fi
	return true;
}
...
void MyViewer::draw() {
	...
	if (show_constraints) {
		...
		for (auto f : keepInContsraint.faces()) {
			...
			glBegin(GL_POLYGON);
			for (auto v : keepInContsraint.fv_range(f)) {
				glNormal3dv(keepInContsraint.normal(v).data());
				glVertex3dv(keepInContsraint.point(v).data());
			}
			glEnd();
		}
		...
		for (auto f : keepOutConstraint.faces()) {
			...
			glBegin(GL_POLYGON);
			for (auto v : keepOutConstraint.fv_range(f)) {
				glNormal3dv(keepOutConstraint.normal(v).data());
				glVertex3dv(keepOutConstraint.point(v).data());
			}
			glEnd();
		}
	}
